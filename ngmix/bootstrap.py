...
class BootstrapperGaussMom(Bootstrapper):
    ...
    def fit_max(self, pars, guess=None, prior=None, extra_priors=None, ntry=1):
        ...
        self.max_fitter = self._fit_one_model_max(
            pars, guess=guess, prior=prior, ntry=ntry
        )
    ...
    def _fit_one_model_max(self, pars, guess=None, prior=None, ntry=1):
        ...
        runner = MaxRunnerGaussMom(
            self.mb_obs_list, pars, guesser, prior=prior
        )
...
class CompositeBootstrapper(Bootstrapper):
    def __init__(self, obs, fracdev_prior=None, fracdev_grid=None, **kw):

        super(CompositeBootstrapper, self).__init__(obs, **kw)

        self.fracdev_prior = fracdev_prior
        if fracdev_grid is not None:
            self.fracdev_tests = linspace(
                fracdev_grid["min"], fracdev_grid["max"], fracdev_grid["num"]
            )
        else:
    ...
    def fit_max(
        ...
        if self.verbose:
            fitting.print_pars(
                dev_fitter.get_result()["pars"], front="        gal_pars:"
            )
            fitting.print_pars(
                dev_fitter.get_result()["pars_err"], front="        gal_perr:"
            )
    ...
    def _maybe_clip(self, efitter, dfitter, pars, fracdev):
        ...
        clip_min = pars.get("fracdev_s2n_clip_min", 1.0e9)
        if s2n_max > clip_min:
            print("        clipping")
    ...
    def _get_gmix_round(self, fitter, pars, band):
        ...
        res = fitter.get_result()
        band_pars = fitter.get_band_pars(pars, band)
        gm_round = GMixCM(res["fracdev"], res["TdByTe"], band_pars)
        return gm_round
    ...
    def _fit_sim_round(
        ...
        res = fitter.get_result()
        runner = CompositeMaxRunnerRound(
            pars_round,
            mb_obs_list,
            max_pars,
            res["fracdev"],
    ...
    def _fit_fracdev(self, exp_fitter, dev_fitter, use_grid=False):
        ...
        if fprior is None:
            ...
            ffitter = FracdevFitterMax(
                self.mb_obs_list, epars, dpars, prior=fprior
            )
            if use_grid:
...
class BestBootstrapper(Bootstrapper):
    def __init__(self, obs, fracdev_prior=None, **kw):
    ...
    def fit_max(self, exp_prior, dev_prior, exp_rate, pars, ntry=1):
        ...
        print("    fitting exp")
        exp_fitter = self._fit_one_model_max(
            "exp", pars, prior=exp_prior, ntry=ntry,
        )
        print("    fitting dev")
        dev_fitter = self._fit_one_model_max(
            "dev", pars, prior=dev_prior, ntry=ntry,
        )
...
class RoundRunnerBase(object):
    def _get_round_guesser(self, **kw):
        ...
        guesser = RoundParsGuesser(self.pars_sub, prior=prior, scaling=scaling)
        return guesser
...
class MaxRunnerRound(MaxRunner, RoundRunnerBase):
    ...
    def __init__(self, pars_full, obs, model, max_pars, *args, **kw):
        ...
        super(MaxRunnerRound, self).__init__(
            obs, model, max_pars, guesser, **kw
        )
...
class CompositeMaxRunner(MaxRunner):
    ...
    def __init__(self, obs, max_pars, guesser, fracdev, TdByTe, prior=None):
        self.obs = obs
...
class CompositeMaxRunnerRound(CompositeMaxRunner, RoundRunnerBase):
    ...
    def __init__(self, pars_full, obs, max_pars, fracdev, TdByTe, **kw):
        ...
        super(CompositeMaxRunnerRound, self).__init__(
            obs, max_pars, guesser, fracdev, TdByTe, **kw
        )
