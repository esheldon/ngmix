...
class FracdevFitterMax(FitterBase):
    def __init__(self, obs, exp_pars, dev_pars, **keys):
        ...
        if self.prior is not None:
            ...
            if self.prior.ndim > 1:
                if len(exp_pars) > 6:
                    raise RuntimeError(
                        "currently only joint " "fracdev prior single band"
                    )
    ...
    def _go_lm(self, fracdev_guess):
        ...
        n_prior_pars = 0
        result = run_leastsq(
            self._calc_fdiff, fracdev_guess, n_prior_pars, **self.pars
        )
    ...
    def _go_nm(self, fracdev_guess):
        ...
        result = minimize_neldermead(
            self._calc_neg_lnprob, fracdev_guess, **self.pars
        )
    ...
    def _set_images(self, exp_pars, dev_pars):
        ...
        for band in range(nb):
            ...
            for iepoch in range(nepoch):
                ...
                eimage = egm.make_image(
                    image.shape, jacobian=epoch_obs.jacobian
                )
                dimage = dgm.make_image(
                    image.shape, jacobian=epoch_obs.jacobian
                )
