...
def test_prepsfmom_error():
    ...
    fitter = PrePSFMom(fwhm=fwhm, pad_factor=1.5)  # test runs faster this way
    ...
    assert np.allclose(np.std(farr), res["flux_err"], atol=0, rtol=0.1)
    assert np.allclose(np.std(Tarr), res["T_err"], atol=0, rtol=0.1)
    assert np.allclose(np.std(g1arr), res["e_err"][0], atol=0, rtol=0.1)
    assert np.allclose(np.std(g2arr), res["e_err"][1], atol=0, rtol=0.1)
