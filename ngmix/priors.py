...
class GPriorBase(PriorBase):
    ...
    def dbyg1_array(self, g1, g2, h=1.0e-6):
        """
    ...
    def dbyg2_array(self, g1, g2, h=1.0e-6):
        """
    ...
    def dlnbyg1_array(self, g1, g2, h=1.0e-6):
        """
        Derivative with respect to g1 at the input g1,g2 location
        Used for lensfit.
    ...
    def dlnbyg2_array(self, g1, g2, h=1.0e-6):
        """
        Derivative with respect to g2 at the input g1,g2 location
    ...
    def get_pqr_num(self, g1in, g2in, s1=0.0, s2=0.0, h=1.0e-6):
        """
        Evaluate
            P
            Q
    ...
    def get_pqrs_num(self, g1in, g2in, h=1.0e-5):
        """
        Evaluate
            P
            Q
        ...
        R12 = (
            (f_p_p - f_p_0 - f_0_p + 2 * f_0_0 - f_m_0 - f_0_m + f_m_m)
            * hsq
            * 0.5
        )
        ...
        S112 = (
            (f_pp_p - 2 * f_0_p + f_mm_p - f_pp_m + 2 * f_0_m - f_mm_m)
            * (1.0 / 8.0)
            * hcub
        )
        S122 = (
            (f_p_pp - 2 * f_p_0 + f_p_mm - f_m_pp + 2 * f_m_0 - f_m_mm)
            * (1.0 / 8.0)
            * hcub
        )
        ...
        for ishear in range(nshear):
            ...
            if doring:
                ...
                P_te, Q_te, R_te = self.get_pqr_num(
                    g1s, g2s, s1=s1expand, s2=s2expand, h=h
                )
                ...
                (w,) = numpy.where(
                    (Pscale[first] > 0.0) & (Pscale[second] > 0.0)
                )
                ...
                (w,) = numpy.where(
                    (Pscale[first] > 1.0e-6) & (Pscale[second] > 1.0e-6)
                )
                w = numpy.concatenate((first[w], second[w]))
                print("kept: %d/%d" % (w.size, npair * 2))
                P_te = P_te[w]
            ...
            if doring:
                ...
                print(
                    mess % (s1, s2, g1g2[0], g1g2[1], g1g2_te[0], g1g2_te[1])
                )
            else:
                mess = (
                    "true: %.6f meas: %.6f +/- %.6f "
        ...
        if eps or show:
            ...
            if not doring:
                perr = biggles.SymmetricErrorBarsY(
                    shear1_true, fracdiff, fracdiff_err, color="blue",
                )
                perr_te = biggles.SymmetricErrorBarsY(
                    shear1_true, fracdiff_te, fracdiff_err_te, color="blue",
                )
            ...
            curve = biggles.Curve(
                shear1_true, poly(shear1_true), type="solid", color="black"
            )
            curve.label = r"$\Delta g/g = %.1f g^2$" % coeffs[0]
            plt.add(curve)
            pkey = biggles.PlotKey(
                0.1, 0.9, [pts, pts_te, curve], halign="left"
            )
            plt.add(pkey)
    ...
    def test_pqr_shear_recovery_iter(
        self,
        shear,
        npair,
        niter,
        ...
        for i in range(niter):
            ...
            P, Q, R = self.get_pqr_num(
                g1s, g2s, s1=shear_expand[0], s2=shear_expand[1]
            )
    ...
    def test_lensfit_shear_recovery(
        self,
        smin,
        smax,
        nshear,
        ...
        for ishear in range(nshear):
            ...
            g1s, g2s = shear_reduced(
                g1, g2, shear_true[ishear, 0], shear_true[ishear, 1]
            )
            ...
            for i in range(nshape):
                ...
                glike = TruncatedSimpleGauss2D(
                    g1m[i], g2m[i], sigma, sigma, self.gmax
                )
                ...
                gsamples[:, 0] = rg1
                gsamples[:, 1] = rg2
                lsobj = lensfit.LensfitSensitivity(
                    gsamples, prior, weights=iweights, h=h
                )
            ...
            if not ring:
                ...
                print(
                    "    sens trim leaves %d/%d" % (w.size, nshape),
                    file=stderr,
                )
                nshape = w.size
            ...
            fracdiff[ishear] = (
                shear_meas[ishear, 0] / shear_true[ishear, 0] - 1
            )
        ...
        err = biggles.SymmetricErrorBarsY(
            shear_true[:, 0], fracdiff, fracdiff_err, color="steelblue"
        )
    ...
    def test_pqrs_shear_recovery(
        ...
        for ishear in range(nshear):
            ...
            s1m, s1e, s2m, s2e = self.test_pqrs_shear_one(
                s1,
                s2,
                Winv,
                chunksize,
        ...
        if eps or show:
            ...
            err1 = biggles.SymmetricErrorBarsY(
                shear1_true,
                fracdiff.astype("f8"),
...
class GPriorBA(GPriorBase):
    ...
    def get_pqr(self, g1in, g2in):
        ...
        R11 = (
            ...
            * (
                g1 ** 6
                + g1 ** 4 * (-2 + 2 * g2 ** 2 - 19 * sig2)
                + (1 + g2 ** 2) * sig2 * (-1 + g2 ** 2 - 8 * sig2)
                + g1 ** 2
                * (
                    1
                    + g2 ** 4
        ...
        R22 = (
            ...
            * (
                g2 ** 6
                + g2 ** 4 * (-2 + 2 * g1 ** 2 - 19 * sig2)
                + (1 + g1 ** 2) * sig2 * (-1 + g1 ** 2 - 8 * sig2)
                + g2 ** 2
                * (
                    1
                    + g1 ** 4
        ...
        R12 = (
            fac * g1 * g2 * (80 + omgsq ** 2 * sig4inv + 20 * omgsq * sig2inv)
        )
    ...
    def get_pqr_expand(self, g1in, g2in, s1, s2):
        ...
        expfac = exp(
            (g1 ** 2 + g2 ** 2 - 2 * g1 * s1 + s1 ** 2 - 2 * g2 * s2 + s2 ** 2)
            / (2.0 * sdet * sigma ** 2)
        )
        ...
        bigfac1 = (
            ...
            + 4 * s1 ** 4 * sigma ** 2
            + 4 * s2 ** 4 * sigma ** 2
            + s1 ** 2 * (1 + 8 * s2 ** 2 * sigma ** 2)
        )
        bigfac2 = (
            s2sqmo + 8 * s2 ** 2 * sigma ** 2 + s1 ** 2 * (1 + 8 * sigma ** 2)
        )
        ...
        R11_fac1 = (
            5 * s1 ** 6
            + 3 * s1 ** 4 * (-7 + 3 * s2 ** 2)
            + 3 * s1 ** 2 * (6 - 6 * s2 ** 2 + s2 ** 4)
            - s2 ** 2 * (2 - 3 * s2 ** 2 + s2 ** 4)
        )
        R11_fac2 = (
            9 - 15 * s2 ** 2 + 6 * s2 ** 4 + s1 ** 2 * (-7 + 6 * s2 ** 2)
        )
        ...
        R11 = (
            (
                ...
                * (
                    ...
                    + 3 * s1sq
                    + s2 ** 2
                    - 4 * g2 * s2 * (-1 + 3 * s1 ** 2 + s2 ** 2)
                    - 2
                    * g2 ** 2
                    * (1 - 9 * s1 ** 2 + 6 * s1 ** 4 + s2 ** 2 - 2 * s2 ** 4)
                    + 4
                    * g2 ** 3
                    * s2
                    * (
            ...
            + (
                ...
                * ssqmo ** 2
                * (
                    (4 * (-1 + 3 * s1 ** 2 + s2 ** 2)) / expfac
                    + (8 * gsqmo * s1 * ssqmo * bigfac4)
                    / (expfac * sdet2 * sigma ** 2)
                    + (
                        gsqmo
                        * ssqmo ** 2
                        * (
            ...
            - (
                ...
                * (
                    ...
                    * (
                        ...
                        - s2 ** 2
                        + 4 * sigma ** 2
                        + 8 * g2 ** 2 * (s1 ** 2 + 3 * s2 ** 2) * sigma ** 2
                        + 2 * g2 * s2 * (ssqmo - 8 * sigma ** 2)
                        - 2 * g2 ** 3 * s2 * bigfac2
                        + g2 ** 4 * bigfac1
                    )
                )
            )
            / (expfac * sdet7 * sigma ** 2)
        )
            ...
            (
                ...
                * (
                    ...
                    * (
                        ...
                        - 2 * g2 * s2 * (-9 + 7 * s2 ** 2)
                        + g2 ** 2
                        * (-18 * s2 ** 2 + 21 * s2 ** 4 - 5 * s2 ** 6)
                        - s1 ** 4
                        * (2 - 12 * g2 * s2 + 3 * g2 ** 2 * (1 + s2 ** 2))
                        + s1 ** 2
                        * (
                            1
                            + 6 * g2 * s2 * (-5 + 2 * s2 ** 2)
                            + g2 ** 2 * (2 + 18 * s2 ** 2 - 9 * s2 ** 4)
                        )
                    )
                )
            )
                    ...
                    + (
                        ...
                        * (
                            ...
                            * (
                                ...
                                * g1
                                * s1
                                * (2 + g2 ** 2 * (1 + s1 ** 2 - 3 * s2 ** 2))
                                + g2 ** 2 * (5 * s1 ** 2 - 3 * (1 + s2 ** 2))
                                + g1 ** 2
                                * (
                                    1
                                    - 3 * s2 ** 2
                                    - 6 * g2 ** 2 * s2 ** 2
                                    + s1 ** 2 * (5 + 2 * g2 ** 2 - 6 * g2 * s2)
                                    + 2 * g2 * s2 * (3 + s2 ** 2)
                                )
                            )
                            * sigma ** 2
            ...
            - (
                ...
                * (
                    ...
                    * (
                        ...
                        - s2 ** 2
                        + 4 * sigma ** 2
                        + 8 * g1 ** 2 * (3 * s1 ** 2 + s2 ** 2) * sigma ** 2
                        + 2 * g1 * s1 * (ssqmo - 8 * sigma ** 2)
                        - 2 * g1 ** 3 * s1 * bigfac2
                        + g1 ** 4 * bigfac1
                    )
                )
        ...
        R12_fac1 = (
            2 * g1 ** 4 * s1 * s2
            + g1 ** 3 * s2 * (-2 - 3 * s1 ** 2 + s2 ** 2)
            + g1 ** 2
            * s1
            * (4 * s2 + 4 * g2 ** 2 * s2 + g2 * (-2 + s1 ** 2 - 3 * s2 ** 2))
            + g2
            * s1
            * (
                -1
            ...
            + g1 * (2 * g2 - s2 + g2 ** 2 * s2 * (-2 - 3 * s1 ** 2 + s2 ** 2))
        )
        R12 = (
            gsqmo ** 2
            * ssqmo ** 2
            * (
                ...
                / sdet2
                + (8 * bigfac3 * bigfac4) / sdet2
                - (16 * (-g1 + g1 ** 2 * s1 + g2 ** 2 * s1) * s2 * ssqmo)
                / sdet
                - (16 * s1 * (-g2 + g1 ** 2 * s2 + g2 ** 2 * s2) * ssqmo)
                / sdet
                + (16 * s1 * bigfac3) / sdet
                + (16 * s2 * bigfac4) / sdet
                - (8 * ssqmo * R12_fac1) / sdet2
                ...
                / (sdet3 * sigma ** 2)
                + (4 * gsqmo * s1 * ssqmo * bigfac3) / (sdet2 * sigma ** 2)
                + (4 * gsqmo * s2 * ssqmo * bigfac4) / (sdet2 * sigma ** 2)
                - (2 * gsqmo * ssqmo ** 2 * R12_fac1) / (sdet3 * sigma ** 2)
            )
        ) / (expfac * sdet4)
