...
class TestFitting(unittest.TestCase):
    ...
    def get_obs_data(self, model, noise, mask=False):
        ...
        sigma = np.sqrt((self.T + self.Tpsf) / 2.0)
        dims = [2.0 * 5.0 * sigma] * 2
        ...
        npsf = rng.normal(scale=self.noisepsf, size=im_psf.shape,)
        im_psf[:, :] += npsf
        wt_psf = np.zeros(im_psf.shape) + 1.0 / self.noisepsf ** 2

        im_obj = gm.make_image(dims, jacobian=j)
        n = rng.normal(scale=noise, size=im_obj.shape,)
        im_obj[:, :] += n
        wt_obj = np.zeros(im_obj.shape) + 1.0 / noise ** 2
        ...
        psf_obs = Observation(im_psf, weight=wt_psf, jacobian=j,)
        ...
        for noise in [0.001]:
            ...
            for ngauss in [1, 2, 3, 4]:
                ...
                runner = bootstrap.EMRunner(
                    obs, Tguess, ngauss, em_pars, rng=self.rng,
                )
                runner.go(ntry=2)
    ...
    def testMetacalGetAll(self):
        ...
        for psf in ["gauss", "fitgauss"]:
            ...
            assert len(odict) == len(
                metacal.METACAL_MINIMAL_TYPES
            ), 'got wrong types for psf="%s": %s' % (psf, repr(odict.keys()))

            for t in metacal.METACAL_MINIMAL_TYPES:
                assert t in odict, 'missing metacal type for psf="%s": %s' % (
                    psf,
                    t,
                )
